...
to setup-people
  set recoveryChance (infectionChance / k)
  create-turtles numberOfPeople
  [
    setxy ((-1 * latticeSize) + random (2 * latticeSize)) ((-1 * latticeSize) + random (2 * latticeSize))
    set cured? false
    set infected? false
    ...
    if (random-float 1 < chanceInitiallyInfected)
    [
      set infected? true
...
to move
  if random-float 1 < diffusionRate
  [ move-to one-of neighbors4]
end

to infect
   let nearby-uninfected (turtles-on patch pxcor pycor)
     with [ not infected? and not cured? ]

     if nearby-uninfected != nobody
     [ ask nearby-uninfected
       [ if random-float 1 < infectionChance
         [ set infected? true
         ]
       ]
     ]
...
to maybe-recover
  if random-float 1 < recoveryChance
    [ set infected? false
      set cured? true
      set susceptible? false
    ]
...
"" ""
PENS
"num_cured" 1.0 0 -14439633 true "" "plot (count turtles with [cured?])"
"num_infected" 1.0 0 -2674135 true "" "plot (count turtles with [infected?])"
"num_susceptible" 1.0 0 -13345367 true "" "plot (count turtles with [susceptible?])"
...
206
Pct Recovered
(count turtles with [cured?]) / (count turtles)
17
1
11
...
<experiments>
  <experiment name="experiment" repetitions="20" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>(count turtles with [cured?]) / (count turtles)</metric>
    <enumeratedValueSet variable="diffusionRate">
      <value value="0.8"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="numberOfPeople">
      <value value="1000"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="latticeSize">
      <value value="100"/>
    </enumeratedValueSet>
    <steppedValueSet variable="k" first="1" step="2" last="100"/>
    <steppedValueSet variable="infectionChance" first="0.05" step="0.05" last="1"/>
    <enumeratedValueSet variable="chanceInitiallyInfected">
      <value value="0.01"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
