...
if modelChoice == 0
    ...
    Dr = 6 * Dt / (8*(particleDiameter/2)^2);       % rad^2/s
%     Dr = 5;
    v = ones(numActives,1);                      % 1e-5 m/s
else
    numActives = 20;
    numPassives = 100;
...
passiveXY = GeneratePassiveXY(numPassives,environmentSize,particleDiameter);
...
markerWidth = 1;
writerObj = VideoWriter('Q2_very_clumpy.avi');
writerObj.FrameRate = 50;
open(writerObj);
for t = 1:T
    for n = activeParticles
        torque(n) = CalculateTorque(xy{t},theta(n),passiveXY,n,T0,rC);
    end
    if modelChoice == 0
    ...
    if mod(t,50) == 0 || t == 1
        if numTrailsToPlot > 0
            ...
            posX = zeros(length(lastTs),numActives);
            posY = posX;
            for iT=1:length(lastTs)
                for j=activeParticles
                    posX(iT,j) = xy{lastTs(iT)}(j,1);
                    posY(iT,j) = xy{lastTs(iT)}(j,2);
                end
            end
            y = plot(posX,posY,'b.');
            hold on
            scat = [[xy{t}(:,1) xy{t}(:,2)]; [passiveXY]];
...
%             iGrays = numActives:(numActives+numPassives);
    ...
    xy{t+1} = UpdateXY(xy{t},passiveXY,dx,dy,environmentSize,particleDiameter);
    theta = theta + dtheta;
end
close(writerObj);